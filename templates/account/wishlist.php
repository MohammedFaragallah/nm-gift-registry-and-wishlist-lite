		...
		<div id="nmgr-account-tabs" class="nmgr-tabs nmgr-account-sections">
			<ul class="nmgr-tab-wrapper">
				<?php foreach ( $tabs as $key => $atab ) : ?>
					<li id="<?php echo esc_attr( $atab[ 'tab_id' ] ); ?>" class="nmgr-tab <?php echo esc_attr( $key ); ?>" tabindex="0" role="tab" aria-controls="tab-<?php echo esc_attr( $key ); ?>">
						<a href="#<?php echo esc_attr( $atab[ 'tab_content_id' ] ); ?>"><?php echo nmgr_kses_post( apply_filters( 'nmgr_account_tab_title', esc_html( $atab[ 'title' ] ), $key, $wishlist ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped          ?></a>
					</li>
				<?php endforeach; ?>
			</ul>
			<div class="nmgr-tab-content-wrapper">
				<?php foreach ( $tabs as $key => $atab ) : ?>
					<div id="<?php echo esc_attr( $atab[ 'tab_content_id' ] ); ?>" class="nmgr-section-content nmgr-tab-content <?php echo esc_attr( $key ); ?>" role="tabpanel">
						<?php
