...
<div id="nmgr-overview"
		...
		<div class="nmgr-statistics">
			...
			<?php if ( nmgr_get_option( 'display_item_quantity', 1 ) ) : ?>
				<a href="#nmgr-tab-items" class="nmgr-go-to-tab stat items-total nmgr-background-color <?php echo $wishlist->is_fulfilled() ? esc_attr( 'highlight' ) : ''; ?>">
					<?php
					...
					printf( _n( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							...
							'nm-gift-registry-lite' ), esc_html( nmgr_get_type_title() ) );
					?>
				</a>
			<?php endif; ?>

			<?php if ( nmgr_get_option( 'display_item_purchased_quantity', 1 ) ) : ?>
				<a href="#nmgr-tab-items" class="nmgr-go-to-tab stat items-purchased nmgr-background-color <?php echo $wishlist->is_fulfilled() ? esc_attr( 'highlight' ) : ''; ?>">
					<?php
					if ( $wishlist->is_fulfilled() ) {
						echo nmgr_get_svg( array( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							'icon' => 'heart',
			...
			<div class="nmgr-share-wishlist nmgr-text-center nmgr-background-color nmgr-status-box">
				...
				<div class="permalink-wrapper">
					...
					<div class="link-actions">
						<a href="#"
							 class="nmgr-action button nmgr-copy nmgr-tip"
							 role="button"
							 title="<?php
							 /* translators: %s: wishlist type title */
						...
						<a class="nmgr-action button nmgr-tip"
							 href="<?php echo esc_url( $wishlist->get_permalink() ); ?>"
							 title="<?php
							 /* translators: %s: wishlist type title */
