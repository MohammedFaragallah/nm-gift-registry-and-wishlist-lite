...
if (isset($grouped_products) && !empty($grouped_products)) :
  foreach ($grouped_products as $grouped_product) :
    if (
      $grouped_product->is_purchasable() &&
...
<div class="option-row">
  ...
  <div class="nmgr-btn-group">
    <div class="nmgr-btn">
      <input type="checkbox" value="1" id="<?php echo esc_attr($field_id); ?>"
        name="nmgr_qty[<?php echo esc_attr($grouped_product->get_id()); ?>]">
      <label for="<?php echo esc_attr($field_id); ?>" class="title" title="<?php
                                                                                  printf(
                                                                                    /* translators: %s: wishlist type title */
                                                                                    esc_html__('Add this item to my %s', 'nm-gift-registry-lite'),
