...
class NMGR_Order {
			...
			if ( $item_in_cart ) {
				...
				if ( $quantity > $desired_item_qty ) {
					...
					$message = apply_filters( 'nmgr_validate_wishlist_item_cart_quantity_message', sprintf(
							...
							__( 'Please choose a quantity of %1$s that is not greater than the quantity requested in the %2$s.', 'nm-gift-registry-lite' ), $item_title, nmgr_get_type_title() ), $quantity, $item, $wishlist );
					wc_add_notice( $message, 'error' );
				}
			}
		}
	...
	public static function validate_product_is_wishlist_item_cart_quantity( $passed, $values, $quantity ) {
		...
		if ( $product->get_stock_quantity() < ($product_cart_quantity + $quantity) ) {
			...
			$message = sprintf( __( 'You cannot add that amount to the cart &mdash; we have %1$s in stock and you already have %2$s in your cart.', 'nm-gift-registry-lite' ),
	...
	public static function notify_if_cart_has_wishlist() {
		...
		if ( nmgr_cart_has_wishlist() ) {
			$item_message = sprintf(
				...
				__( 'There are some items in your cart for a specific %s.', 'nm-gift-registry-lite' ), nmgr_get_type_title() );
	...
	public static function notify_wishlist_shipping() {
		if ( nmgr_cart_has_wishlist() ) {
			wc_print_notice(
				apply_filters( 'nmgr_checkout_shipping_message',
					/* translators: %1$s: wishlist type title, %2$s: wishlist type title */
					sprintf( __( 'Items in your order which are for a %1$s would be shipped separately to the owner of the %2$s.', 'nm-gift-registry-lite' ),
