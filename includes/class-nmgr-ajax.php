...
class NMGR_Ajax {
	...
	public static function save_profile() {
		...
		try {
			...
			if ( $form->has_errors() ) {
				...
				wc_add_notice(
					/* translators: %s: wishlist type title */
					sprintf( __( 'Your %s details have been saved', 'nm-gift-registry-lite' ),
						nmgr_get_type_title() ),
					'success' );
		...
		foreach ( $product_objects as $product_object ) {
			...
			if ( $managing_stock && !empty( $_GET[ 'display_stock' ] ) ) {
				...
				$formatted_name .= ' &ndash; ' . sprintf(
						...
						__( 'Stock: %d', 'nm-gift-registry-lite' ), wc_format_stock_quantity_for_display( $stock_amount, $product_object ) );
			}
	...
	public static function dialog_create_new_wishlist() {
		...
		$args = array(
			'title' => sprintf(
				/* translators: %s: wishlist type title */
				__( 'Create new %s', 'nm-gift-registry-lite' ), nmgr_get_type_title()
			),
