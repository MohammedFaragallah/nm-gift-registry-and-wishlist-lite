...
class NMGR_Wordpress {
	...
	public static function enable_wishlist() {
		...
		$enabled_html = sprintf( '<a href="%s" tabindex="1" class="button wc-forward">%s</a> %s',
		...
		try {
			...
			if ( $product->is_type( 'grouped' ) ) {
				...
				if ( empty( $posted_quantities ) ) {
					/* translators: %s: wishlist type title */
					throw new Exception( sprintf( esc_html__( 'Please choose the quantity of items you wish to add to your %s.', 'nm-gift-registry-lite' ),
							esc_html( nmgr_get_type_title() )
						) );
				}
