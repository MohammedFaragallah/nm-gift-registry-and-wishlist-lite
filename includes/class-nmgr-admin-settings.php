				...
				if ( isset( $field[ 'checkboxgroup' ] ) ) {
					...
					if ( $group_fields ) {
						foreach ( $group_fields as $group_field ) {
							printf( "<label><input %s value='%s' name='%s' type='checkbox' /> %s</label><br />", // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					...
					printf( "<label><input %s value='1' name='%s' type='checkbox' %s /> %s</label>", // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						self::checked( $field ),
						$field_name,
						$field_custom_attributes, // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					...
					foreach ( $field[ 'options' ] as $key => $args ) :
						...
						<div class="nmgr-btn">
							...
							<label for="<?php echo esc_attr( $option_id ); ?>"
										 title="<?php echo isset( $args[ 'label_title' ] ) ? esc_attr( $args[ 'label_title' ] ) : ''; ?>"
										 class="nmgr-tip">
											 <?php
			...
			'fields' => array(
				...
				array(
					...
					'desc_tip' => sprintf(
						...
						__( 'This would allow users view a wishlist at a url like %1$s/%2$s/sample-wishlist/. It is best to set this only once to keep the name consistent for users.', 'nm-gift-registry-lite' ), get_bloginfo( 'url' ), nmgr_get_option( 'permalink_base', 'nm-gift-registries' ) ),
				),
				array(
			...
			'fields' => array(
				...
				array(
					...
					'options' => array(
						'button' => array(
							...
							'image' => sprintf( '<div class="button" disabled>%s</div>',
				...
				array(
					...
					'default' => sprintf(
						...
						__( 'Add to %s', 'nm-gift-registry-lite' ), esc_html( nmgr_get_type_title() )
					),
					'placeholder' => sprintf(
						/* translators: %s: wishlist type title */
						__( 'Add to %s', 'nm-gift-registry-lite' ), esc_html( nmgr_get_type_title() )
	...
	public static function do_settings_sections_full_version() {
		...
		<div class="nmgr-full-version">
			<div class="nmgr-text-center">
				<a class="nmgr-buy-btn" href="https://nmgiftregistry.com/product/nm-gift-registry" rel="noopener noreferrer" target="_blank"><?php esc_html_e( 'Upgrade Now', 'nm-gift-registry-lite' ); ?></a>
			</div>
			...
			<div class="nmgr-features">
				<?php foreach ( $features as $feature ) : ?>
					<div class="nmgr-feature">
						<div class="nmgr-image"><?php echo $feature[ 'image' ]; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped                                                        ?></div>
						<div class="nmgr-info">
