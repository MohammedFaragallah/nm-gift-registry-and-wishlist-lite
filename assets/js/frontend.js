...
jQuery(function($) {
    ...
    jQuery(function($) {
        ...
        var add_to_wishlist = {
             ...
             * Response object should contain 'items_data' key which is an array of objects containing for each item removed,
             * - wishlist_item_id
             * - wishlist_id
             * - product_id
            ...
            removed_from_wishlist: function(e, response) {
                ...
                if (response && response.items_data) {
                    response.items_data.forEach(function(item) {
                        var form = $('.nmgr-add-to-wishlist-form-' + item.product_id);
                        if (form.length > 0) {
                            if (!item.product_in_wishlist) {
    ...
    jQuery(function($) {
        ...
        var wishlist_cart = {
            init: function() {
                $(document.body)
                    .on('nmgr_added_to_wishlist nmgr_items_reloaded', this.reload)
            ...
            remove_wishlist_item: function(e) {
                ...
                if (window.confirm($(this).attr('data-notice'))) {
                    nmgr.items.delete_items([$(this).data('wishlist-item-id')], $(this).data('wishlist-id'));
                }
            },
            ...
            add_to_wc_cart: function(e) {
                ...
                $.each($(this).data(), function(index, obj) {
                    // Camelcase to dashed
                    item[index.replace(/([a-zA-Z])(?=[A-Z])/g, '$1-').toLowerCase()] = obj;
                });
